...
function RatnaChart() {
    ...
    useEffect(() => {
        const fetchdata = () => {
            fetch('https://zomat0-datas.onrender.com/mumbai').then(data => {
                ...
                for (var i of res.ratnagiri.barChart) {
                    labelBar.push(i.year);
                    dataBar.push(i.turnover);
                }

                for (var j of res.ratnagiri.lineChart) {
    ...
    var options = {
        ...
        plugins: {
            legend: { position: "top" },
            title: { display: true, text: " Ratnagiri - Zomato Turnover FY 21-23" }
