...
function KoraChart() {
    ...
    useEffect(() => {
        const fetchdata = () => {
            fetch('https://zomat0-datas.onrender.com/bengaluru').then(data => {
                ...
                for (var i of res.koramangala.barChart) {
                    labelBar.push(i.year);
                    dataBar.push(i.turnover);
                }

                for (var j of res.koramangala.lineChart) {
    ...
    var options = {
        ...
        plugins: {
            legend: { position: "top" },
            title: { display: true, text: " Koramangala - Zomato Turnover FY 21-23" }
