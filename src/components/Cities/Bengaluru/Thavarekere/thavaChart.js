...
function ThavaChart() {
    ...
    useEffect(() => {
        const fetchdata = () => {
            fetch('https://zomat0-datas.onrender.com/bengaluru').then(data => {
                ...
                for (var i of res.thavarekere.barChart) {
                    labelBar.push(i.year);
                    dataBar.push(i.turnover);
                }

                for (var j of res.thavarekere.lineChart) {
    ...
    var options = {
        ...
        plugins: {
            legend: { position: "top" },
            title: { display: true, text: "Thavarekere - Zomato Turnover FY 21-23" }
