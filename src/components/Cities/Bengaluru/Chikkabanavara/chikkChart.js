...
function ChikkChart() {
    ...
    useEffect(() => {
        const fetchdata = () => {
            fetch('https://zomat0-datas.onrender.com/bengaluru').then(data => {
                ...
                for (var i of res.chikkabanavara.barChart) {
                    labelBar.push(i.year);
                    dataBar.push(i.turnover);
                }

                for (var j of res.chikkabanavara.lineChart) {
    ...
    var options = {
        ...
        plugins: {
            legend: { position: "top" },
            title: { display: true, text: " Chikkabanavara - Zomato Turnover FY 21-23" }
