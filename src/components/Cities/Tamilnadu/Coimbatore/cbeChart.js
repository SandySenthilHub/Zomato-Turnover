...
function CbeChart() {
    ...
    useEffect(() => {
        const fetchdata = () => {
            fetch(' https://zomat0-datas.onrender.com/tamilnadu').then(data => {
                ...
                for (var i of res.coimbatore.barChart) {
                    labelBar.push(i.year);
                    dataBar.push(i.turnover);
                }

                for (var j of res.coimbatore.lineChart) {
    ...
    var options = {
        ...
        plugins: {
            legend: { position: "top" },
            title: { display: true, text: " Coimbatore - Zomato Turnover FY 21-23" }
