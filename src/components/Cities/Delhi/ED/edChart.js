...
import { Chart as ChartJs, ArcElement, CategoryScale, LinearScale, BarController, BarElement, PointElement, LineElement, Title, Tooltip, Legend, Filler } from 'chart.js';
...
function EdChart() {
    ...
    useEffect(() => {
        const fetchdata = () => {
            fetch('https://zomat0-datas.onrender.com/delhi').then(data => {
                ...
                for (var i of res.ed.barChart) {
                    labelBar.push(i.year);
                    dataBar.push(i.turnover);
                }

                for (var j of res.ed.lineChart) {
    ...
    var options = {
        ...
        plugins: {
            legend: { position: "top" },
            title: { display: true, text: " East Delhi - Zomato Turnover FY 21-23" }
